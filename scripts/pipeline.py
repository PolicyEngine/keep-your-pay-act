"""Data generation pipeline for KYPA calculator dashboard.
...
def _extract_distributional(result: dict, variant: str, year: int) -> list[dict]:
    ...
    for decile, avg in result["decile"]["average"].items():
        ...
        if "decile_tax" in result:
            row["average_change_tax"] = round(
                result["decile_tax"]["average"][decile], 2
            )
            row["relative_change_tax"] = round(
                result["decile_tax"]["relative"][decile], 6
            )
        rows.append(row)
    return rows
...
def _run_year_in_process(year: int) -> dict:
    """Run reform for a single year, then free memory."""
...
def _run_year_subprocess(year: int) -> dict:
    """Run reform for one year in a subprocess to isolate memory."""
